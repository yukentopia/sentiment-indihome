...
# ==========================================
# 🔒 SISTEM AUTENTIKASI (LOGIN) VIA DATABASE
# ==========================================
...
if not st.session_state['logged_in']:
    ...
    st.markdown("""
        <style>
            /* Sembunyikan sidebar sepenuhnya dari halaman login */
            [data-testid="collapsedControl"] { display: none; }
            [data-testid="stSidebar"] { display: none; }
            
            /* Jarak atas agar posisi login pas di tengah layar */
            .block-container {
                padding-top: 5rem !important;
            }
            
            /* Kustomisasi tombol login agar serasi dengan tema IndiHome */
            .stButton > button {
                width: 100%;
    ...
    # --- AUTO-SETUP TABEL USER DI MYSQL ---
    try:
        ...
        # Cek apakah tabel masih kosong. Jika kosong, isi dengan 1 user admin saja
        cursor.execute("SELECT COUNT(*) FROM users")
        if cursor.fetchone()[0] == 0:
            cursor.execute("INSERT INTO users (username, password, nama_lengkap) VALUES (%s, %s, %s)", ("admin", "admin", "Administrator"))
            conn.commit()
            
        cursor.close()
        conn.close()
    except Exception as e:
        st.warning(f"⚠️ Gagal menghubungkan atau membuat tabel user: {e}")

    # --- LAYOUT KOTAK LOGIN DI TENGAH ---
    col_kiri, col_tengah, col_kanan = st.columns([1, 1.2, 1])
    
    with col_tengah:
        with st.container(border=True):
            # Taruh logo di dalam kotak
            c1, c2, c3 = st.columns([1, 2, 1])
            with c2:
                st.image("logo_indihome.png", use_container_width=True)
            
            st.markdown("<h3 style='text-align: center; color: #262730; margin-bottom: 25px;'>Portal Analisis Sentimen</h3>", unsafe_allow_html=True)
            
            with st.form("login_form", border=False):
                # --- TAMBAHKAN PARAMETER value="admin" AGAR AUTO-FILL ---
                username = st.text_input("👤 Username", value="admin", placeholder="Masukkan username...")
                password = st.text_input("🔒 Password", value="admin", type="password", placeholder="Masukkan password...")
                # --------------------------------------------------------
                
                st.markdown("<br>", unsafe_allow_html=True)
                
                # --- TRIK MENG TENGAHKAN TOMBOL ---
                col_btn_kiri, col_btn_tengah, col_btn_kanan = st.columns([1, 2, 1])
                ...
                if submit_button:
                    # --- CEK KREDENSIAL KE DATABASE SQL ---
                    try:
                        conn = get_db_connection()
                        cursor = conn.cursor()
                        
                        cursor.execute("SELECT nama_lengkap FROM users WHERE username = %s AND password = %s", (username, password))
                        user_data = cursor.fetchone()
                        
                        if user_data:
                            st.session_state['logged_in'] = True
                            st.session_state['nama_user'] = user_data[0] 
                            st.rerun() 
                        else:
                            st.error("❌ Username atau Password salah!")
                            
                    except Exception as e:
                        st.error(f"Terjadi kesalahan saat memverifikasi login: {e}")
                    finally:
...
# ==========================================
with st.sidebar:
    # --- KODE SAKTI CSS (SUPER AGRESIF UNTUK TOP & BOTTOM) ---
    st.markdown(
        """
        <style>
            /* 1. MENGHILANGKAN SPACE ATAS TOTAL */
            [data-testid="stSidebarUserContent"] {
                padding-top: 0rem !important;
                margin-top: -20px !important; /* Tarik paksa ke atas jika masih ada sisa */
            }
            
            /* 2. MENGHILANGKAN MARGIN GAMBAR BAWAAN STREAMLIT */
            ...
            /* 3. MENGURANGI JARAK GARIS PEMBATAS */
    ...
    # --- KODE MENU DENGAN PEMBUNGKUS NATURAL (FIX CROPPED BOTTOM) ---
    menu = option_menu(
        ...
        # 2. URUTAN IKON DISESUAIKAN DENGAN MENU
        icons=[
            "house",           # untuk Dashboard
            "database",        # untuk Dataset
            "gear",            # untuk Preprocessing
            "tags",            # untuk Labeling
            "cloud",           # untuk Word Cloud
            "calculator",      # untuk TF-IDF
            "bar-chart-line",  # untuk Modeling & Evaluation
            "clock-history",   # untuk History & Comparison
            "robot"            # untuk Prediksi Model
        ], 
...
# ----------------- 2. DATASET -----------------
    ...
    with tab_aksi:
        ...
        # ========================================================
        # FITUR BARU: HAPUS SEMUA FILE .PKL
        # ========================================================
        with st.expander("🧹 Bersihkan File Model (.pkl)"):
            st.warning("Tindakan ini akan menghapus semua file model dan vectorizer (TF-IDF) yang sebelumnya sudah disimpan ke dalam direktori aplikasi.")
            
            if st.button("🗑️ Hapus Semua File .pkl", type="primary", key="btn_hapus_pkl"):
                import os
                import glob
                
                # Mencari semua file berakhiran .pkl di folder tempat app.py berada
                file_pkl = glob.glob("*.pkl")
                ...
                if file_pkl:
                    ...
                    st.success(f"✅ Berhasil menghapus {berhasil_dihapus} file model (.pkl)!")
                    # Opsional: st.rerun() jika kamu ingin halamannya langsung refresh
                else:
                    st.info("💡 Tidak ditemukan file .pkl di dalam sistem saat ini.")
                    
    # ========================================================
    # TAB 3: UPLOAD & OLAH CSV BARU (Dengan Pengecekan Database)
    # ========================================================
    with tab_upload:
        ...
        try:
            ...
            jumlah_data_db = cursor.fetchone()[0]
        except Exception as e:
            jumlah_data_db = 0
            st.error(f"Gagal mengecek database: {e}")
        finally:
            cursor.close()
            conn.close()

        # --- 2. LOGIKA WAJIB HAPUS ---
        if jumlah_data_db > 0:
            st.warning(f"⚠️ **Sistem terkunci:** Ditemukan **{jumlah_data_db} baris data** di database aktif.")
            st.write("Untuk menjaga integritas model, kamu **wajib menghapus** seluruh data yang ada saat ini sebelum mengunggah dataset baru.")
            
            # Tombol hapus paksa dengan warna merah (primary)
            if st.button("🗑️ Hapus Seluruh Dataset Saat Ini", type="primary", use_container_width=True):
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("TRUNCATE TABLE dataset_tweets")
                conn.commit()
                ...
                # Bersihkan juga session jika masih ada nyangkut
                st.session_state['current_dataset'] = None 
                
                st.success("✅ Database berhasil dikosongkan! Memuat ulang...")
                st.rerun()
                
        # --- 3. JIKA DATABASE KOSONG, TAMPILKAN ALUR UPLOAD SEPERTI BIASA ---
        else:
            if 'current_dataset' in st.session_state and st.session_state['current_dataset'] is not None:
                ...
                if st.button("Simpan Dataset ke MySQL", key="btn_simpan_db_final"):
                    ...
                    with st.spinner("Menyimpan data ke database..."):
                        # Truncate di sini hanya untuk safety net, aslinya db sudah dipastikan kosong
                        cursor.execute("TRUNCATE TABLE dataset_tweets")
                ...
                st.info("Sistem siap. Belum ada file CSV yang diunggah.")
...
# ----------------- 7. MODELING & EVALUATION -----------------
    ...
    st.write("### 📊 Eksperimen Klasifikasi")
    st.info("Pilih algoritma yang ingin dilatih. Model yang kamu pilih di sini akan dievaluasi dan otomatis diekspor (.pkl) agar bisa digunakan di menu Prediksi Model.")
    ...
    with col_model:
        selected_models = st.multiselect(
            "Pilih Model (Bisa lebih dari satu):", 
            ["Support Vector Machine (SVM)", "Naive Bayes", "Decision Tree", "Random Forest"],
            default=["Support Vector Machine (SVM)"]
        )
    ...
    if use_smote:
        st.info("SMOTE Aktif: Data latih akan diseimbangkan secara sintetis.")
    else:
        st.warning("SMOTE Nonaktif: Model akan dilatih dengan distribusi data asli.")
        
    st.markdown("---")
    
    if st.button("🚀 Mulai Training & Simpan Model", type="primary", key="btn_klasifikasi"):
        if len(selected_models) == 0:
            st.warning("⚠️ Pilih minimal satu model untuk dilatih!")
        elif df.empty or df['label'].isnull().all() or (df['label'].astype(str).str.strip() == '').all():
            st.error("❌ Data/Label masih kosong. Silakan preprocessing dan labeling terlebih dahulu!")
        else:
            import json 
            import pickle
            ...
            my_bar = st.progress(0, text="Menyiapkan data (TF-IDF)...")
            
            # 1. Generate & Simpan TF-IDF Vectorizer
            ...
            # 2. Siapkan Data Full untuk Ekspor Model Final
            if use_smote:
                smote_export = SMOTE(random_state=42)
                X_train_final, y_train_final = smote_export.fit_resample(X_tfidf, y)
            else:
                X_train_final, y_train_final = X_tfidf, y
            
            # Dictionary untuk mapping nama model ke algoritma scikit-learn
            model_mapping = {
                "Support Vector Machine (SVM)": ("svm", SVC(kernel='linear', random_state=42)),
            ...
            for kolom, tipe in kolom_tambahan.items():
                try:
                    cursor.execute(f"ALTER TABLE model_evaluation ADD COLUMN {kolom} {tipe}")
                    conn.commit()
                except:
                    pass 
            
            total_model = len(selected_models)
            
            # --- MULAI LOOPING UNTUK SETIAP MODEL YANG DIPILIH ---
            for i, model_choice in enumerate(selected_models):
                my_bar.progress(int((i / total_model) * 100), text=f"Melatih & Menyimpan {model_choice} ({status_text})... Mohon tunggu.")
                
                # A. EVALUASI K-FOLD (Masuk Database)
                ...
                try:
                    cursor.execute(
                        "INSERT INTO model_evaluation (model_name, k_fold, accuracy, precision_score, recall_score, f1_score, use_smote, f1_macro, balanced_acc, mcc, confusion_matrix) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                        (model_choice, k_fold, float(acc), float(prec), float(rec), float(f1_weighted), use_smote, float(f1_macro), float(bal_acc), float(mcc), cm_json)
                    )
                except Exception as db_err:
                    st.error(f"Gagal menyimpan riwayat evaluasi {model_choice}: {db_err}")
                    
                # B. SIMPAN FILE .PKL HANYA UNTUK MODEL INI
                file_prefix, clf = model_mapping[model_choice]
                clf.fit(X_train_final, y_train_final) # Latih dengan 100% data
                
                # Penamaan file disesuaikan dengan toggle SMOTE
                file_name = f"model_{file_prefix}_smote.pkl" if use_smote else f"model_{file_prefix}.pkl"
                with open(file_name, 'wb') as f:
                    pickle.dump(clf, f)
                    
            conn.commit()
            cursor.close()
            my_bar.progress(100, text="Proses training & ekspor selesai!")
            
            st.success("🎉 Proses Selesai! Model yang kamu pilih berhasil dievaluasi (cek menu History) dan file `.pkl`-nya sudah diperbarui untuk Prediksi.")
    conn.close()
...
# ----------------- 8. HISTORY & COMPARISON -----------------
    ...
    if df_eval.empty:
        ...
        # --- MEMBUAT TABS ---
        tab_tabel, tab_grafik, tab_cm, tab_hapus = st.tabs([
            "📜 Tabel Riwayat", 
            "📊 Grafik Perbandingan", 
            "🧮 Confusion Matrix",
            "⚙️ Pengaturan"
        ...
        with tab_tabel:
            ...
            rename_dict = {
                'id': 'ID', 'model_name': 'Algoritma', 'k_fold': 'K-Fold', 'use_smote': 'Pakai SMOTE', 
                'accuracy': 'Akurasi', 'precision_score': 'Presisi', 'recall_score': 'Recall', 
                'f1_score': 'F1-Score', 'f1_macro': 'Macro F1', 'balanced_acc': 'Balanced Acc', 'mcc': 'MCC'
            }
            df_display = df_display.rename(columns=rename_dict)
            
            kolom_urutan = [c for c in ['ID', 'Algoritma', 'K-Fold', 'Pakai SMOTE', 'Akurasi', 'Presisi', 'Recall', 'F1-Score', 'Macro F1', 'Balanced Acc', 'MCC'] if c in df_display.columns]
        ...
        with tab_grafik:
            ...
            metric_map = {
                "Akurasi": "accuracy", "Presisi": "precision_score", "Recall": "recall_score",
                "F1-Score (Weighted)": "f1_score", "Macro F1": "f1_macro", "Balanced Accuracy": "balanced_acc", "MCC": "mcc"
            }
            ...
            if 'use_smote' in df_eval.columns:
                df_eval['Model_Label'] = df_eval.apply(lambda x: f"{x['model_name']} (SMOTE) [ID:{x['id']}]" if x['use_smote'] else f"{x['model_name']} [ID:{x['id']}]", axis=1)
            else:
            ...
            if selected_metric_col in df_eval.columns and not df_eval[selected_metric_col].isnull().all():
                ...
                if selected_metric_col == 'mcc': plt.ylim(-1.0, 1.1)
                elif df_eval[selected_metric_col].max() > 1.0: plt.ylim(0, 105)
                else: plt.ylim(0, 1.1)
                ...
                for p in ax.patches:
                    ...
                    if not np.isnan(val):
                        ...
                        ax.annotate(format(val, fmt), (p.get_x() + p.get_width() / 2., val), ha='center', va='center', xytext=(0, 9), textcoords='offset points')
                st.pyplot(fig)
            else:
                st.warning("Data metrik belum tersedia.")
        ...
        with tab_cm:
            ...
            st.info("Pilih ID Eksperimen untuk melihat bentuk Confusion Matrix-nya secara detail.")
            
            if 'confusion_matrix' in df_eval.columns:
                options = df_eval.apply(lambda x: f"ID {x['id']} - {x['model_name']} ({'SMOTE' if x['use_smote'] else 'Tanpa SMOTE'})", axis=1).tolist()
                
                col_cm1, col_cm2 = st.columns([1, 1])
                with col_cm1:
                    selected_cm_option = st.selectbox("Pilih Eksperimen:", options)
                
                if selected_cm_option:
                    selected_id = int(selected_cm_option.split(" ")[1])
                    cm_str = df_eval[df_eval['id'] == selected_id]['confusion_matrix'].values[0]
                    
                    if pd.notna(cm_str) and cm_str != "":
                        import json
                        import numpy as np
                        try:
                            cm_array = np.array(json.loads(cm_str))
                            ...
                            st.error(f"Format Confusion Matrix tidak valid: {e}")
                    else:
                        st.warning("Confusion Matrix tidak tersimpan pada eksperimen ini (Mungkin data lama).")
            else:
                st.warning("Kolom Confusion Matrix belum ada di database. Silakan jalankan 1 eksperimen baru di menu 'Modeling'.")

        # ==========================================
        # TAB 4: PENGATURAN / HAPUS RIWAYAT
        # ==========================================
        with tab_hapus:
...
# ----------------- 9. PREDIKSI MODEL (TESTING) -----------------
    ...
    # --- MEMBUAT TABS UNTUK PREDIKSI ---
    tab_uji, tab_riwayat = st.tabs(["🧪 Uji Prediksi", "🕒 Riwayat Prediksi"])

    # ==========================================
    # TAB 1: FORM UJI PREDIKSI
    # ==========================================
    with tab_uji:
        ...
        if st.button("🚀 Prediksi Sentimen", type="primary"):
            if test_text.strip() == "":
                ...
                # CEK KETERSEDIAAN FILE TF-IDF
                if not os.path.exists('tfidf_vectorizer.pkl'):
                    st.error("⚠️ File sistem (TF-IDF Vectorizer) tidak ditemukan. Pastikan kamu sudah melatih dan menyimpan model `.pkl` di menu 'Modeling & Evaluation' terlebih dahulu.")
                else:
                    ...
                    for i, (file_name, display_name) in enumerate(model_names.items()):
                        with cols_no_smote[i]:
                            ...
                            model_path = f'model_{file_name}.pkl'
                            ...
                            if os.path.exists(model_path):
                                with open(model_path, 'rb') as f:
                                    ...
                                    hasil_rekaman[file_name] = pred_bersih 
                    ...
                    for i, (file_name, display_name) in enumerate(model_names.items()):
                        with cols_smote[i]:
                            ...
                            model_smote_path = f'model_{file_name}_smote.pkl'
                            
                            if os.path.exists(model_smote_path):
                                with open(model_smote_path, 'rb') as f:
                                    model = pickle.load(f)
                                    pred = model.predict(text_tfidf)[0]
                                    
                                    pred_bersih = str(pred).strip().lower()
                                    hasil_rekaman[file_name + "_smote"] = pred_bersih 
                                ...
                                st.info("⚪ Belum tersedia")
                                hasil_rekaman[file_name + "_smote"] = "-"
    ...
    # ==========================================
    # TAB 2: RIWAYAT PREDIKSI DENGAN PREVIEW & EXPORT PDF
    # ==========================================
    with tab_riwayat:
        st.write("### 🕒 Tabel Riwayat Uji Coba Prediksi")
        ...
        try:
            ...
            if not df_history.empty:
                ...
                df_display['waktu'] = df_display['waktu'].astype(str) 
                ...
                # Fungsi untuk membuat PDF menggunakan FPDF
                def create_pdf(dataframe):
                    pdf = FPDF(orientation='L', unit='mm', format='A4')
                    pdf.add_page()
                    pdf.set_font("Arial", 'B', 14)
                    pdf.cell(0, 10, "Laporan Riwayat Prediksi Model Sentimen", ln=True, align='C')
                    pdf.ln(5)
                    
                    col_widths = [45, 65, 17, 17, 17, 17, 25, 25, 25, 25] 
                    ...
                    for row in dataframe.itertuples(index=False):
                        for i, item in enumerate(row):
                            text_val = str(item)[:40] + '...' if i == 1 and len(str(item)) > 40 else str(item)
                ...
                # Generate data PDF di memori
                pdf_data = create_pdf(df_display)
                
                # --- MEMBUAT 3 KOLOM UNTUK TOMBOL ---
                col_preview, col_btn_dl, col_btn_del = st.columns([1.5, 1.5, 1])
                
                with col_preview:
                    # Saklar untuk memunculkan preview
                    show_preview = st.toggle("👁️ Tampilkan Preview PDF")
                
                with col_btn_dl:
                    st.download_button(
                        label="📄 Download PDF Sekarang",
                        data=pdf_data,
                        file_name="Laporan_Riwayat_Prediksi.pdf",
                ...
                # --- LOGIKA UNTUK MENAMPILKAN PREVIEW PDF ---
                if show_preview:
                    st.markdown("### 🔍 Preview Dokumen")
                    import base64
                    # Mengubah bytes PDF menjadi base64 string
                    base64_pdf = base64.b64encode(pdf_data).decode('utf-8')
                    # Membuat elemen iframe HTML untuk menampilkan PDF
                    pdf_viewer = f'<iframe src="data:application/pdf;base64,{base64_pdf}" width="100%" height="600" type="application/pdf"></iframe>'
                    st.markdown(pdf_viewer, unsafe_allow_html=True)
                    
            else:
                st.info("Belum ada riwayat prediksi yang dilakukan. Silakan coba prediksi di tab sebelah!")
                
        except Exception as e:
            st.error(f"Terjadi kesalahan saat memuat/mengekspor riwayat: {e}")
